...
import { TRANSACTION_SIZE_LIMIT, V1_TRANSACTION_SIZE_LIMIT } from '../transaction-size';
...
describe('getTransactionMessageSizeLimit', () => {
    it('returns LEGACY_TRANSACTION_SIZE_LIMIT for a legacy transaction message', () => {
        ...
        expect(getTransactionMessageSizeLimit(legacyMessage)).toBe(TRANSACTION_SIZE_LIMIT);
    });

    it('returns LEGACY_TRANSACTION_SIZE_LIMIT for a v0 transaction message', () => {
        expect(getTransactionMessageSizeLimit(SMALL_TRANSACTION_MESSAGE)).toBe(TRANSACTION_SIZE_LIMIT);
    });
